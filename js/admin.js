...
function switchTab(tabId) {
    ...
    const titleMap = { 'kelolaBarang': 'Manajemen Barang', 'pengaturanBanner': 'Pengaturan Banner Promosi', 'pengaturanToko': 'Pengaturan Toko & Kontak', 'logAktivitas': 'Riwayat Aktivitas (CCTV)' };
...
function updateLabelVariasi() {
    ...
    items.forEach((item, index) => {
        item.querySelector('.label-nomor-variasi').textContent = `Variasi ${index + 1}`;
    });
    document.getElementById('teksTambahVariasi').textContent = `Tambah Variasi ${items.length + 1}`;
}
